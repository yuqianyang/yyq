...
<table class="table">
    ...
    <c:forEach items="${couponDatas.pages }" var="item" varStatus="status">
        <tr>
            ...
            <td>${item.cardnum }</td>
            <td>${item.time }</td>
            <td><input class="btn btn-default" type="button" onclick="findCouponById(${item.id })" value="查看"><input
                    class="btn btn-default" type="button" onclick="deleteCouponInfo(${item.id})" value="删除"></td>
        </tr>
    </c:forEach>
</table>
<ul class="pagination">
...
<script type="text/javascript">
    ...
    function addCoupon() {
        var html = "<label>优惠券金额：</label><div style=\"width: 30%;\">"
            ...
            + "<label>停车卡号：</label><div style=\"width: 30%;\">"
            + "<div class=\"input-group\">"
            + "<input id=\"cardnum\" name=\"cardnum\" type=\"text\" class=\"form-control\" readonly=readonly unselectable=\"on\">"
            + "</div></div>"
            + "<label>是否随机发放：</label>"
            + "<select onchange=\"cr()\" id=\"changeRandom\" name=\"changeRandom\" style=\"width:100px\" class=\"form-control\"> "
    ...
    function cr() {
        var changeRandom1 = $("#changeRandom").val();
        if (changeRandom1 == 1) {
            $("#cardnum").val('');
