...
<table class="table">
    <caption>
                    ...
                    <input id="cardnum2" placeholder="请输入卡号" type="text" class="form-control"> <span
                        class="input-group-btn">
								<button class="btn btn-default" type="button"
                                        onclick="findDepotcardByCardnum()">查询</button>
							</span>
                </div>
                <!-- /input-group -->
            </div>
        ...
        <button style="float: right; margin-right: 30px" class="btn btn-default" type="button"
                onclick="rechargeDepotCard()">充值停车卡
        </button>
    </caption>
    <tr>
        <th>序号</th>
    ...
    <c:forEach items="${depotcardManagerDatas.pages }" var="item" varStatus="status">
        <tr>
            ...
            <td><input class="btn btn-default" type="button" onclick="alertCard(${status.count})" value="修改"><c:if
                    test="${sessionScope.user.role!=3 }"><input class="btn btn-default" type="button"
                                                                onclick="deleteCard(${status.count})" value="删除"></c:if>
            </td>
        </tr>
    </c:forEach>
</table>
<ul class="pagination">
    ...
    <li>
        <a href="${APP_PATH }/index/toDepotcardIndex?tag=${depotcardManagerDatas.tag}&&page=${depotcardManagerDatas.current}&&cardnum=${depotcardManagerDatas.extra}"
           target="main"
           onclick="$('div#main').load(this.href);return false;">&laquo;</a></li>
    <li>
        <a href="${APP_PATH }/index/toDepotcardIndex?tag=${depotcardManagerDatas.tag}&&page=${depotcardManagerDatas.current+1}&&cardnum=${depotcardManagerDatas.extra}"
           target="main"
    ...
    <c:if test="${depotcardManagerDatas.current+1<depotcardManagerDatas.countPage}">
        <li>
            <a href="${APP_PATH }/index/toDepotcardIndex?tag=${depotcardManagerDatas.tag}&&page=${depotcardManagerDatas.current+2}&&cardnum=${depotcardManagerDatas.extra}"
               target="main"
               onclick="$('div#main').load(this.href);return false;">&raquo;</a></li>
    </c:if>
</ul>
            ...
            success: function (data) {
                ...
                if (data.code == 100) {
                    ...
                    var html = "<input id=\"payid\" name=\"payid\"  value=\"1\" hidden=\"hidden\"/>"
                        + "<label>账号：</label><div style=\"width: 30%;\">"
                        + "<div class=\"input-group\">"
                        + "<input id=\"username\" name=\"username\" placeholder=\"请输入账号\" type=\"text\" class=\"form-control\">"
                        + "</div>"
                        + "</div>"
                        + "<label>手机：</label><div style=\"width: 30%;\">"
                        + "<div class=\"input-group\">"
                        + "<input id=\"tel\" name=\"tel\" placeholder=\"请输入手机\" type=\"text\" class=\"form-control\">"
                        + "</div>"
                        + "</div>"
                        + "<label>姓名：</label><div style=\"width: 30%;\">"
                        + "<div class=\"input-group\">"
                        + "<input id=\"name\" name=\"name\" placeholder=\"请输入姓名\" type=\"text\" class=\"form-control\">"
                        + "</div>"
                        + "</div>"
